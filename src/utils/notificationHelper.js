...
export function createBookingNotification(
  ...
  const notification = {
    ...
    status: status,
    title: isConfirmed ? "✅ Đặt bàn đã được xác nhận!" : "❌ Đặt bàn bị hủy",
    message: isConfirmed
      ? `Đặt bàn tại ${booking.restaurantName || "nhà hàng"} vào ${
          booking.date
        } lúc ${
          booking.time
        } đã được xác nhận. Chúc bạn có trải nghiệm tuyệt vời!`
      : `Đặt bàn tại ${booking.restaurantName || "nhà hàng"} đã bị hủy. ${
          cancelReason
            ? `Lý do: ${cancelReason}`
